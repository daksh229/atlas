"""
schema.py — the canonical SQLite schema for atlas.db, plus the per-table column
order used when bulk-loading.

Table and field names mirror the corresponding Odoo concepts (res.partner →
partners, sale.order → sale_orders, purchase.order → purchase_orders, product →
products, stock → inventory) so a future read-only Odoo integration is a near
drop-in.

Access model is trader-owned: partners.owner_trader_id is the account owner the
§9 access control enforces on.

`build_db.py` imports SCHEMA and COLUMNS from here to create and populate the DB.
"""
...
CREATE TABLE supply_signals (
    id            TEXT PRIMARY KEY,
    brand_id      TEXT REFERENCES brands(id),
    partner_id    TEXT REFERENCES partners(id),   -- nullable for offer-sourced supply
    trader_id     TEXT REFERENCES traders(id),
    offer_price   REAL,
    available_qty INTEGER,
    fired_at      TEXT,
    source        TEXT                              -- purchase_history | manual_offer | email_offer
);
